...
function TaskRow({
  ...
  async function saveLabel() {
    ...
    setSaving(true);
    const res = await fetch(`/api/maintenance/${jobId}/progress-items/${item.id}`, {
      method: "PATCH",
...
function CategorySection({
  ...
  async function deleteCategory(e: React.MouseEvent) {
    ...
    setDeleting(true);
    const res = await fetch(`/api/maintenance/${jobId}/progress-categories/${category.id}`, { method: "DELETE" });
    setDeleting(false);
  ...
  async function toggleItem(item: any) {
    ...
    await fetch(`/api/maintenance/${jobId}/progress-items/${item.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteItem(item: any) {
    ...
    await fetch(`/api/maintenance/${jobId}/progress-items/${item.id}`, { method: "DELETE" });
  }

  async function addTask() {
    if (!newTask.trim()) return;
    const res = await fetch(`/api/maintenance/${jobId}/progress-items`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export default function EditMaintenanceModal({
  ...
  const [form, setForm] = useState({
    ...
    shopDrawingStatus: job.shopDrawingStatus ?? "NOT_STARTED",
    jobType: job.jobType ?? "REACTIVE",
    description: job.description ?? "",
    contractValue: job.contractValue?.toString() ?? "",
    receivedAmount: job.receivedAmount?.toString() ?? "",
  });
  const [categories, setCategories] = useState<any[]>(job.progressCategories ?? []);
  const [saving, setSaving] = useState(false);
  const [deleting, setDeleting] = useState(false);
